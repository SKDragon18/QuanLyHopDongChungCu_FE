...
const DieuKhoanSelector = ({dieuKhoanUpdateList, setDieuKhoanUpdateList}) => {
    const getCode = ()=>{
    ...
    const handleAddDieuKhoanMoi = ()=>{
        if(dieuKhoanMoi.noiDung !== ""){
            setDieuKhoanUpdateList([...dieuKhoanUpdateList,dieuKhoanMoi])
            setDieuKhoanMoi({
                ma:getCode(),
                noiDung:''
            })
    ...
    const handleSubDieuKhoanMoi = (ma)=>{
        setDieuKhoanUpdateList(dieuKhoanUpdateList.filter(dieuKhoan=>dieuKhoan.ma!==ma))
    }
    ...
    return (
        ...
        <table className='table table-bordered table-hover'>
              ...
              <tbody>
                {(!Array.isArray(dieuKhoanUpdateList)||dieuKhoanUpdateList.length===0)?
                (
                  <tr>
                    <td colSpan="7" className='text-center'>Danh sách rỗng</td>
                  </tr>
                ):(dieuKhoanUpdateList.map((dieuKhoan)=>(
...
export default DieuKhoanSelector
