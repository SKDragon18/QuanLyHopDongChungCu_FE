...
import { getBanQuanLyById, updateBanQuanLy, doiHinhAnh, getKhachHangById } from '../utils/ApiFunctions'
import { useParams } from 'react-router-dom'
import {Link} from 'react-router-dom'
const EditTaiKhoan = () => {
    const {role,tenDangNhap} = useParams()
    ...
    useEffect(()=>{
        const fetchTaiKhoanById= async()=>{
            try{
                if(role==="khachhang"){
                    const result = await getKhachHangById(tenDangNhap);
                    setTaiKhoan(result)
                    setChangeable(true)
                }
                else{
